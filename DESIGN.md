...
| Layer | Rule |
| --- | --- |
| Drawing | Inline SVG, one 44×28 viewBox read as a **44×28 pixel grid**. The middle 30 cells are the cup itself — dome head over a fluted wrapper, four ridges, two arm poses with 2×2 hands, seven sparkles — and the margins either side are empty until a busy kit fills them. No asset, no image request. |
...
| Busy kits | Going busy also picks one of four **working scenes** at random, written to `body[data-busy]`: `phone` (landline on a side desk, receiver at the ear, cord swinging), `keys` (monitor with a blinking caret and a keyboard, both hands on it), `bowl` (whisk turning in a mixing bowl), `board` (a bar chart growing on an easel). Each kit is two groups — `.kit.floor` for the furniture, which sits in the margin and holds still, and `.kit.held` for the arm and prop, which rides the cup's bob inside `.rig`. A kit draws its own arms, so `.arms` and the two sparkles it would collide with step aside while it shows. |
| Kit tempo | Every kit animates the same way the cup does: a `.f1`/`.f2` pair trading opacity on `steps(1, end)`, never a tween. Only the duration differs, and it is the character — whisking at .22s, typing at .3s, the phone cord at .6s, the board bars at 1.1s. |
| Caption | Generated by CSS `content` per state — *warm and idle* / *that went sideways* — and per kit while busy: *on it — on the phone* / *typing it up* / *whisking* / *at the board*. Lowercase small caps, `--ink-faint`. |
| Wiring | JS sets exactly two words: `document.body.dataset.agent` and, on the edge into busy, `dataset.busy`. Every visual consequence is a stylesheet rule, matching the harness rule that JS never sets style. `?busy=bowl` pins one kit for a look. |
