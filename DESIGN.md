...
| Mood | The colour and face come from `body[data-agent]`: idle → `--ink-dim` + grin, sparkles dark; busy → `--warn` + open mouth + sparkles lit and twinkling; error → `--bad` + flat mouth + smaller eyes, sparkles dark. |
...
| Mascot sparkles † | 1.3s loop, `scale(.35)`+`rotate(-30°)` → `scale(1)`+`rotate(20°)`, **busy only** | `ease-in-out` |
...
`no-preference` guard and always run. The reasoning: it is a 5rem line drawing
that breathes 3px in place — no travel, no parallax, no scroll coupling, nothing
