...
| Layer | Rule |
| --- | --- |
| Drawing | Inline SVG, one 120×108 viewBox: dome head over a fluted wrapper, four ridges, two waving arms with round hands, seven sparkles. No asset, no image request. |
| Sparkles | One four-point star in `<defs>`, placed seven times with `<use>` at scales .5–1.1 in an arc over the head and shoulders. Solid `currentColor` fill — the `#sparkle` id rule outranks the blanket `.cup *` stroke, so they never render as hairline outlines. |
...
| Mood | The colour and face come from `body[data-agent]`: idle → `--ink-dim` + grin + half-lit sparkles; busy → `--warn` + open mouth + full sparkles at double speed; error → `--bad` + flat mouth + smaller eyes + sparkles off. |
...
| Mascot sparkles | 2.6s loop, `scale(.35)`+`rotate(-30°)` → `scale(1)`+`rotate(20°)` | `ease-in-out` |

Sparkle delays are **negative** (−0.35s … −2.1s), so each star starts mid-cycle
and the group twinkles out of phase from the first frame — never a synchronized
pulse, which would read as a spinner.

While `body[data-agent="busy"]`, the breath drops to 1.1s, the wave to 0.7s, and
the sparkles to 1.1s — same keyframes, faster clock. The mascot never gains new
motion when working; it only gets more excited about the motion it already had.
